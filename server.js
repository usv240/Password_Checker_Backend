...
const userDataPatterns = require('./userData'); // Import patterns from userData.js
const app = express();
...
function detectPredictablePatterns(password) {
    const patterns = [
        'qwerty', 'asdf', 'zxcv', '12345', '67890', 'abcd',
        'password', 'admin', 'letmein', 'welcome'
    ];
    return patterns.filter(pattern => password.toLowerCase().includes(pattern));
}
...
function detectCommonMutations(password) {
    ...
    return detectedMutations.length > 0 ? detectedMutations : false;
}
...
function detectPasswordPrediction(password, userData) {
    ...
    if (userData) {
        ...
        if (birthdate) {
            const formattedBirthdate = birthdate.replace(/-/g, ''); // Remove dashes
            if (password.includes(formattedBirthdate)) {
                matchedPatterns.push(formattedBirthdate);
            }
        }

        // Check for parts of the email
        if (email) {
            const emailParts = email.split(/[@.]/); // Split by '@' and '.'
    ...
    // Check against default patterns from userData.js
    userDataPatterns.forEach((pattern) => {
        if (typeof pattern === 'string' && password.toLowerCase().includes(pattern.toLowerCase())) {
            matchedPatterns.push(pattern);
        }
    });

    isGuessable = matchedPatterns.length > 0;
    return { isGuessable, matchedPatterns };
}
...
function calculatePasswordScore(password) {
    ...
    return Math.min(Math.max(score, 0), 100); // Clamp score between 0 and 100
}
...
function getPasswordFeedback(password, userData) {
    ...
    if (!/[!@#$%^&*(),.?":{}|<>]/.test(password)) tips.push("Use special characters like @ or #.");
...
app.post('/check-password', async (req, res) => {
    ...
    // Hash and store the user data along with the password
